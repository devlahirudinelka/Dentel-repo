...
import  PictureAsPdIcon  from "@material-ui/icons/PictureAsPdf";
...
function createPdf(pdfBody){
    ...
    doc.save('feedback.pdf'); //this downloads a copy of the pdf in your local instance.
};
...
<><div className ="feedbackBody">
    ...
    <div className="container">
        ...
        <div>
            <input className="searchfeedback"
            style={{ width:"15%" ,height:"30px"}}
            type="text"
            placeholder="search..."
            onChange={(e) => setSearchName(e.target.value)}/></div>
        <table id="my-table" class="table table-bordered feedbackTable table-light">
            ...
            {loading ? (
                    ...
                    posts
                        .filter((value) => {
                            if (searchName === "") {
                                ...
                                value.patient_name.includes(searchName.toLowerCase())
                            ){
                                return value;
                            }
                    ...
                    <td>{feedbacks.patient_name}</td>
                    <td>{feedbacks.diagnosis}</td>
                    <td>{feedbacks.dentist_name}</td>
                    <td>{feedbacks.rating}</td>
                    <td>{feedbacks.feedback_description}</td>
                    

                    <Link to={`/EditFeedback/${feedbacks._id}`} class="btn btn-success btn-sm"><EditIcon/>Update</Link>
                    &nbsp;
                    <button type="button" class="btn btn-danger btn-sm" onClick={() => {if (window.confirm('Are you sure you wish to delete this record?')) deleteFeedback(feedbacks._id)}} ><DeleteIcon/>Delete
                    </button>
       ...
       <Link to={"/AddFeedback"} className="btn btn-success btn-sm"><AddCircleIcon/>Add New Feedback</Link>
       &nbsp;
