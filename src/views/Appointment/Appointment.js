...
export default function Appointment() {
    ...
    function createPdf(pdfBody){
        ...
        doc.autoTable({
            didDrawPage: function (data) {
                ...
                // Header
                doc.setFontSize(14);
                var fileTitle = "Appointment Report";
    ...
    return (
        ...
        <table id="my-table" class="table table-bordered appointmentTable table-light">
            ...
            {loading ? (
                    ...
                    posts
                        .filter((value) => {
                            if (searchName === "") {
                                ...
                                value.patient_name.includes(searchName.toUpperCase())
                            ){
                                return value;
                            }
                ...
                <tr key={appointments._id}>
                    ...
                    <td>{appointments.dentist_name}</td>
                    <td>{appointments.phone}</td>
                    <td>{appointments.appointment_date}</td>
                    <td>{appointments.appointment_time}</td>
                    

                    <Link to={`/EditAppointment/${appointments._id}`} class="btn btn-success btn-sm"><EditIcon/>Update</Link>
                    &nbsp;
                    <button type="button" class="btn btn-danger btn-sm" onClick={() => {if (window.confirm('Are you sure you wish to delete this record?')) deleteAppointment(appointments._id)}} ><DeleteIcon/>Delete
                    </button>
       ...
       <Link to={"/AddAppointment"} className="btn btn-success btn-sm"><AddCircleIcon/>Add New Appointment
       </Link> 
       &nbsp;
