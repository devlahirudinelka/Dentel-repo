...
export default function Payment() {
    ...
    function deletePayment(_id) {
        axios.delete("http://localhost:8070/payment/delete/"+_id).then((res) =>{
        ...
        <table id="my-table" class="table table-bordered appointmentTable table-light">
            ...
            {loading ? (
                ...
                <tr key={payments._id}>
                    ...
                    <td>{payments.diagnosis}</td>
                    <td>{payments.hospitalCharge}</td>
                    <td>{payments.doctorFee}</td>
                    <td>{payments.entryDate}</td>
